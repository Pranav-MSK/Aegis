...
class RequestStrategy(ABC):
    ...
    @abstractmethod
    def execute(self, client: BaseAPIClient, endpoint: str, **kwargs):
...
class GetRequest(RequestStrategy):
    ...
    def execute(self, client: BaseAPIClient, endpoint: str, **kwargs):
        return client.request("GET", endpoint, params=kwargs.get('params'))


class PostRequest(RequestStrategy):
    """ Strategy for POST requests. """
    def execute(self, client: BaseAPIClient, endpoint: str, **kwargs):
        return client.request("POST", endpoint, json=kwargs.get('data'))


class PutRequest(RequestStrategy):
    """ Strategy for PUT requests. """
    def execute(self, client: BaseAPIClient, endpoint: str, **kwargs):
        return client.request("PUT", endpoint, json=kwargs.get('data'))
    

class DeleteRequest(RequestStrategy):
    """ Strategy for DELETE requests. """
    def execute(self, client: BaseAPIClient, endpoint: str, **kwargs):
        return client.request("DELETE", endpoint, params=kwargs.get('params'))
